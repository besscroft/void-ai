...
export async function getMemory(): Promise<Memory | null> {
  ...
  memoryInstance = new Memory({
    ...
    vectorStore: { provider: "memory", config: { collectionName: "void-memories", dimension: 1536 } },
    historyDbPath,
  });
...
export async function searchMemoriesSemantic(
  ...
  if (!memory) {
    ...
    return listMemories()
      .filter((m) => m.scope === "global" || m.agent_id === agentId ||
...
async function searchMemories(
  ...
  agentId?: string | null,
  conversationId?: string,
): Promise<Array<{ id: string; scope: MemoryScope; kind: MemoryKind; title: string; content: string; salience: number; pinned: boolean }>> {
  const { searchMemoriesSemantic } = await import("./mem0-service");
