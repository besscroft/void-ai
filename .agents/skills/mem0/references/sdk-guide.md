...
// OR across agents
filters: {
  OR: [{ user_id: "alice" }, { agent_id: { in: ["travel-agent", "sports-agent"] } }];
}

// Category filtering (partial match)
filters: {
  AND: [{ user_id: "alice" }, { categories: { contains: "finance" } }];
}

// Category filtering (exact match)
filters: {
  AND: [{ user_id: "alice" }, { categories: { in: ["personal_information"] } }];
}
```
